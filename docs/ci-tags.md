...
the continuous integration behaviour of your merge requests.

## Current CI tags

* `[ci:skip-build]`: Do not build modified packages. Instead, only verify
...
* `[ci:skip-dint]`: Do not run the device-linter check. Use this when moving
  or modifying many devices that are not maintained by you and, due to
  historical reasons, might not pass the check.
...
* `[ci:skip-kconfigcheck]`: Do not check the kernel config for packages
  ...
  categories or performing bulk changes across a lot of kernel packages. Never
  use this when upgrading the kernel.
